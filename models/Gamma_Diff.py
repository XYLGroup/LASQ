...
class Net(nn.Module):
    ...
    def gamma_energy(self, gamma_map, img_float):
        enhanced = torch.clamp(img_float.pow(gamma_map), 0, 1)
        exposure_target = 0.55
        exposure_loss = torch.mean(
            torch.abs(enhanced.mean(dim=[1, 2, 3]) - exposure_target)
        )

        dh = torch.mean(torch.abs(gamma_map[:, :, 1:, :] - gamma_map[:, :, :-1, :]))
        dw = torch.mean(torch.abs(gamma_map[:, :, :, 1:] - gamma_map[:, :, :, :-1]))
        smooth_loss = dh + dw

        grad_input_h = torch.mean(torch.abs(img_float[:, :, 1:, :] - img_float[:, :, :-1, :]))
        grad_input_w = torch.mean(torch.abs(img_float[:, :, :, 1:] - img_float[:, :, :, :-1]))
        grad_output_h = torch.mean(torch.abs(enhanced[:, :, 1:, :] - enhanced[:, :, :-1, :]))
        grad_output_w = torch.mean(torch.abs(enhanced[:, :, :, 1:] - enhanced[:, :, :, :-1]))
        structure_loss = torch.abs((grad_input_h + grad_input_w) - (grad_output_h + grad_output_w))

        energy = exposure_loss + 0.1 * smooth_loss + 0.1 * structure_loss
        return energy

    def mcmc_sample_gamma(self, img_float, init_gamma, num_steps=4, proposal_std=0.03, temperature=0.02):
        gamma_current = init_gamma.clone()
        energy_current = self.gamma_energy(gamma_current, img_float)
        ...
        for _ in range(num_steps):
            ...
            accept_prob = torch.exp(-delta / temperature)
            rand_val = torch.rand(1).to(img_float.device)

            if rand_val < accept_prob:
                gamma_current = proposal
    ...
    def enhance_with_gamma(self, img_tensor, num_regions):
        ...
        # mcmc_steps = getattr(self.args, 'mcmc_steps', 16)
        # mcmc_std = getattr(self.args, 'mcmc_std', 0.03)
        # mcmc_temp = getattr(self.args, 'mcmc_temp', 0.02)

        sampled_gamma = self.mcmc_sample_gamma(
            img_float,
            base_gamma,
            num_steps=self.args.mcmc_steps,
            proposal_std=self.args.mcmc_std,
            temperature=self.args.mcmc_temp
        )

        gamma_img = self.partition_gamma(img_float, sampled_gamma, num_regions)
    ...
    def train(self, DATASET, local_rank=0, world_size=1):
        ...
        for epoch in range(self.start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                x = F.pad(x, (0, img_w_64 - w, 0, img_h_64 - h), 'reflect')
